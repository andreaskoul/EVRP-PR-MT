...
ProblemDefinition = namedtuple('ProblemDefinition',
                               ['size', 'coordinate_points', 'display_coordinate_points',
                                'customer_demands', 'distance_matrix', 'capacity_constraint', 'optimal_value',
                                'stations', 'energy_capacity', 'vehicles', 'edge_weight_type'])
...
def read_TSPLIB_CVRP(file_name):
    ...
    * ewt is the EDGE_WEIGHT_TYPE
    ...
    with open(file_name, "r") as fh:
        ...
        for l in fh.readlines():
            ...
            if ':' in line:
                ...
                if 'TYPE' == field:
                    if not 'CVRP' in value and not 'EVRP' in value and not 'TSP' in value:
                        raise IOError("Only CVRP&EVRP TSPLIB files are supported")
                elif 'DIMENSION' in field:
                    N = int(value) - 1  # depot excluded
                elif 'ENERGY_CAPACITY' in field:
                    energy_capacity = int(value)
                elif 'VEHICLES' in field:
                    vehicles = int(value)
                elif 'CAPACITY' in field:
                    C = int(value)
                elif 'OPTIMAL_VALUE' in field:
                    optimal = int(value)
                elif 'STATIONS' in field:
                    stations = int(value)
    ...
    return ProblemDefinition(N, points, dd_points, demands, D, C, optimal, stations, energy_capacity, vehicles, edge_weight_type)
