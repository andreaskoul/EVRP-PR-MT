...
def do_2optstar_move(route1_data, route2_data, D, stations_ids=None ,L=None, d=None,
                     C=None,  # constraints
...
@routeordersensitive
def do_1point_move(route1_data, route2_data, D, stations_ids, L=None, d=None,
                   C=None,  # constraints
                   strategy=LSOPT.FIRST_ACCEPT,
                   best_delta=None):
...
def do_2point_move(route1_data, route2_data, D, stations_ids, L=None, d=None,
                   C=None,  # constraints
                   strategy=LSOPT.FIRST_ACCEPT,
                   best_delta=None):
...
@routeordersensitive
def do_insert_move(unrouted, recieving_route_data, D, stations_ids=None, L=None, d=None, C=None,
                   strategy=LSOPT.FIRST_ACCEPT,
                   best_delta=None):
            ...
            for i in range(1, len(ansatz_route)):
                ...
                if not L or ansatz_l + insert_delta - S_EPS <= L:
                    if (best_insert_delta is None) or \
                        ...
                        # FIRST_ACCEPT when customers are inserted one by one
...
def do_redistribute_move(redisributed_route_data,
                         ...
                         D, stations_ids, L=None, d=None, C=None,
                         strategy=LSOPT.FIRST_ACCEPT,
                         best_delta=None,
                         recombination_level=2):
    """
    ...
    * recombination_level=3, all possible orderings of the nodes to be
    ...
    for route_customers, candidate_routes in insertion_node_orderings:
        ...
        for to_insert in route_customers:
            if to_insert in stations_ids:
                continue
            node_succesfully_inserted = False
            for rd2_ar_idx, rd2 in enumerate(ansatz_route_data):
                _, new_rd2, delta = do_insert_move(unrouted=to_insert, recieving_route_data=rd2, D=D,
                                                   stations_ids=stations_ids, d=d, C=C, L=L, strategy=strategy)
                if delta is None:
                    continue
...
@routeordersensitive
def do_chain_move(route1_data, route2_data, route3_data, D, stations_ids, L=None, d=None,
                  C=None,  # constraints
                  strategy=LSOPT.FIRST_ACCEPT,
                  best_delta=None):
    """ This is the "pair" operation described in Wren and Holliday (1972). 
    It involves moving and replacing a node on route 2 with a node on route 1.
